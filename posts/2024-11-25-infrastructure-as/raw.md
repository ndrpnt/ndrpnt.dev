...
* I kinda disagree on the close the gap between code and infra. Both Pulumi automation API and Terraform exec are wrappers around the CLI (in go and typescript respectively). They are not librairies as they should be. And nothing prevent a DSL based tool like terraform to be library first with a thin CLI/DSL wrapper. At its core all tools are written in a general purpose programming language anyway, so any tool correctly architectured (i.e. a library and multiple thin wrappers around it (CLI, controller, …)) closes the gap between IaC and code.
* Both IaS and classic IaC can be used to create higher level abstractions. Only the potential glue code might feel more integrated/natural with IaS, as it can't be expressed in IaC dsl
* Testing and ide integration (navigation completion) are available to both although DSL have to build it from scratch while general purpose programming languages can leverage existing tooling
