...
Operators empower developers to consume managed services using the Kubernetes API, without the need for administrator intervention. They play a central role when building [Internal Developer Platforms](https://internaldeveloperplatform.org/) atop Kubernetes.
...
<figcaption>Example Kubernetes manifest defining a Strimzi Kafka cluster.</figcaption>

A lot of ready-made operators are available on [Artifact Hub](https://artifacthub.io/packages/search?operators=true). Kafka, PostgreSQL, MongoDB, or etcd are all examples of applications for which an operator exists. Be aware that not all of them have truly been battle-tested in production. Likewise, many operators have a major version number of zero (0.y.z) indicating potential frequent API breaking changes, so use them at your own risk.

With previously unattainable levels of automation now one [`helm install`](https://helm.sh) away, it might be tempting to deploy many operators. There is no such thing as a free lunch however, and confidently supporting the kind of service that Strimzi offers requires strong technical knowledge of not only Kubernetes and Kafka, _but Strimzi itself_.

It is however sometimes needed to write its own, for instance, to address a company-specific need…
...
The operator pattern can be applied to a lot of different use cases and is especially appropriate when dealing with infrastructure-related applications and middleware. Having in mind [SRE](https://landing.google.com/sre/) practices, we are convinced that writing operators will (and should) be considered regular cluster administration.

To this end, multiple tools aiming at simplifying the creation of operators already exist and the landscape is still rapidly evolving.
...
[Custom Resources](https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/custom-resources/) work exactly as core Kubernetes resources (like pods, services, deployments, …). The custom objects are stored in etcd and queried through the kube-apiserver (e.g. `kubectl get mycustomresource`).
...
Of course the actual implementation has to be smarter, to prevent infinite loogs hogging CPU and flooding the API server with requests.
...
TODO: ajouter un schéma avec les dépendances Operator SDK, Kubebuilder, controller tools, controller runtime
